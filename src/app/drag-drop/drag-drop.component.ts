...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DiagramModule, TreeViewModule],
...
export class DragDropComponent {
  ...
  public click(args: IClickEventArgs) {
    if (
      args.element &&
      (args.element as any).sourceID === undefined &&
      (args.element as any).shape !== undefined
    ) {
  ...
  public drop(args: any) {
    ...
    setTimeout(() => {
      ...
      tempData = (workingData as any).filter(
        (a: any) => a.Id === this.targetNodeId
      );
      ...
      if (args.element.inEdges.length === 0) {
        ...
        this.treeview.addNodes([item], this.targetNodeId, 0);
  ...
  public addNode(nodeId: any) {
    ...
    if (this.diagram.selectedItems.nodes!.length > 0) {
      ...
      let temp = this.diagram.nodes.filter(
        (a: any) => a.data.Id === this.targetNodeId
      );
      targetId = temp[0].id;
    }
